-- RushHub for Doors
-- Fixed: Freezes, InstanceHandler, Primary Scan, Performance Tab
-- Part 1 of 4
...
function ESPLibrary:RenderObject(Object)
    ...
    if CachedHighlight and CachedHighlight.Enabled then
        ...
        local DistanceText = self.ShowDistance and ("\n" .. '<font size="' .. math.round(self.TextSize * self.DistanceSizeRatio) .. '">[' .. Distance .. ']</font>') or ""
...
Interface.ApplyInfoTab = function(Window)
    ...
    local Changelog = {
        ...
        "<font color='rgb(0, 255, 0)'>+ Added Stairwell tab</font>",
        "<font color='rgb(0, 255, 0)'>+ Added anti-re-execute protection</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Sally Toy to ItemNames</font>",
        "<font color='rgb(0, 255, 0)'>+ Added ArchivesStorageBox ignore in Auto Interact</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Cellar support in AllowedInstances</font>",
        "<font color='rgb(0, 255, 0)'>+ Added FPS & Ping Stats Overlay</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Honcho Correct Box ESP</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Performance Tab</font>",
        "<font color='rgb(255, 255, 0)'>~ Increased Window CornerRadius (2 → 6)</font>",
        "<font color='rgb(255, 255, 0)'>~ Improved unload cleanup</font>",
        "<font color='rgb(255, 255, 0)'>~ Minor polish on Archives prompt handling</font>",
        "<font color='rgb(0, 255, 0)'>+ FIXED: Micro-freezes / stutters during gameplay</font>",
        "<font color='rgb(0, 255, 0)'>+ FIXED: InstanceHandler & Primary Workspace scan</font>",
        "<font color='rgb(0, 255, 0)'>+ FIXED: ESP FPS drop in multiplayer</font>",
        "<font color='rgb(0, 255, 0)'>+ FIXED: Cleaner & PromptFixer switched to task.spawn</font>",
    }
...
function STX:Notify(nofdebug, middledebug, all)
    ...
    elseif SelectedType == "option" then
        ...
        Check.ZIndex = 5
        Check.AutoButtonColor = false        Check.Image = "http://www.roblox.com/asset/?id=6031094667"
...
-- ════════════════════════════════════════════════════════════════
-- TABS — Performance icon: rbxassetid://113052394116996 (спидометр)
-- ════════════════════════════════════════════════════════════════
local Tabs = {
...
-- ════════════════════════════════════════════════════════════════
-- QUEUE PROCESSOR — FPS FIX (batch 3 per tick)
-- ════════════════════════════════════════════════════════════════
...
Connections.QueueConnection = Services.RunService.Heartbeat:Connect(function()
    ...
    if #Globals.ObjectQueue == 0 then return end
    QueueProcessing = true
    task.spawn(function()
        local Processed = 0
        while #Globals.ObjectQueue > 0 and Processed < MAX_PER_TICK do
...
-- ════════════════════════════════════════════════════════════════
-- AUTO INTERACT LOOP — FPS FIX (10Hz throttle, single trigger)
-- ════════════════════════════════════════════════════════════════
...
Connections.AutoInteract = Services.RunService.Heartbeat:Connect(function()
    ...
    for _, Prompt in Objects.Prompts do
        if Prompt:GetAttribute("ParentRoom") and tonumber(Prompt:GetAttribute("ParentRoom")) ~= CurrentRoom then continue end
...
-- ════════════════════════════════════════════════════════════════
-- CLEANER — FPS FIX (task.spawn, 1.5s interval)
-- ════════════════════════════════════════════════════════════════
