-- RushHub for Doors — FPS FIX EDITION
-- Part 1 of 4
-- Fixes: Frustum culling, batched ESP, dynamic queue, weak tables, throttled loops
...
RunTest("isfunctionhooked", function()
    ...
    Global.restorefunction(TestFunction)
    assert(Global.isfunctionhooked(TestFunction) == false, "Did not return false for an unhook function")
end)
...
-- ════════════════════════════════════════════════════════════════
-- ESP LIBRARY — FPS FIX: FRUSTUM CULLING + BATCHED RENDER
-- ════════════════════════════════════════════════════════════════
local ESPLibrary = {
    ...
    RenderAccumulator = 0,
    MaxRenderDistance = 800,  -- skip ESP for objects beyond this distance
    DirtyObjects = false,     -- flag for cleanup
    LastColorUpdate = {},     -- cache last color to avoid redundant property sets
}
...
-- FPS FIX: optimized destroy — batch all cleanup
local function DestroyObjectData(Object)
    ...
    ESPLibrary.LastColorUpdate[Object] = nil
...
-- FPS FIX: frustum check — skip objects outside camera view
local function IsInFrustum(Camera, WorldPos, Margin)
    local CamPos = Camera.CFrame.Position
    local ToObj = (WorldPos - CamPos)
    local Dist = ToObj.Magnitude
    if Dist < 0.001 then return true, 0 end
    local Dir = ToObj / Dist
    local Dot = Camera.CFrame.LookVector:Dot(Dir)
    -- 0.3 radians ~ 72 degrees FOV margin (covers most FOV settings)
    local Threshold = Margin or 0.15
...
function ESPLibrary:RenderObject(Object)
    ...
    -- FPS FIX: use GetPivot safely, cache result
    local PivotOk, ObjectPos = pcall(function() return Object:GetPivot().Position end)
    if not PivotOk or not ObjectPos then return end

    -- FPS FIX: frustum culling + distance culling BEFORE WorldToViewportPoint
    local CamPos = ESPCamera.CFrame.Position
    local Distance = (CamPos - ObjectPos).Magnitude
    if Distance > self.MaxRenderDistance then
        local CachedHighlight = self.Highlights[Object]
        if CachedHighlight and CachedHighlight.Enabled then CachedHighlight.Enabled = false end
        local Frame = self.Frames[Object]
        if Frame and Frame.Visible then Frame.Visible = false end
        local LineData = self.Lines[Object]
        if LineData and LineData[1] and LineData[1].Visible then LineData[1].Visible = false end
        return
    end
    ...
    if not OnScreen then
        ...
        if self.ElementsEnabled[Object] == true then
            ...
            if not CachedHighlight.Enabled then CachedHighlight.Enabled = true end
        end
    ...
    -- FPS FIX: only update color if changed
    local ActiveColor = self.Rainbow and RainbowState.Color or self.ColorTable[Object] or Color3.fromRGB(255, 255, 255)
    local LastColor = self.LastColorUpdate[Object]
    if LastColor ~= ActiveColor then
        self.LastColorUpdate[Object] = ActiveColor
    ...
    if CachedHighlight and CachedHighlight.Enabled then
        if self.ShowDistance and Label then
            local DistText = "\n" .. '<font size="' .. math.round(self.TextSize * self.DistanceSizeRatio) .. '">[' .. math.floor(Distance) .. ']</font>'
            Label.Text = self.TextTable[Object] .. DistText
        elseif Label and Label.Text ~= self.TextTable[Object] then
            Label.Text = self.TextTable[Object]
        end
    ...
    -- FPS FIX: only compute tracer if enabled AND on screen
    if LineData and self.Tracers == true and OnScreen then
    ...
    elseif LineData and LineData[1] and LineData[1].Visible then
    ...
    if self.Arrows == true then
        ...
        elseif Arrow and self.ElementsEnabled[Object] == true then
            if OnScreen and ScreenPoint.Z > 0 then
                ...
                if not Arrow.Visible then Arrow.Visible = true end
                Arrow.ImageColor3 = self.Rainbow and self.RainbowColor or self.ColorTable[Object] or Color3.fromRGB(255, 255, 255)
            end
        end
    end
end

-- FPS FIX: render loop with inline dead-object cleanup
function ESPLibrary:StartRenderLoop()
    ...
    self.RenderConnection = ESPRunService.Heartbeat:Connect(function(Delta)
        ...
        local TotalCount = #self.TotalObjects
        if TotalCount == 0 then return end

        local CleanupBatch = 0
        local MaxCleanupPerFrame = 5  -- FPS FIX: limit cleanup per frame

        for _, Object in ipairs(self.TotalObjects) do
            if self.ElementsEnabled[Object] == true then
                -- FPS FIX: inline validity check, mark for removal
                if not Object or not Object.Parent then
                    self.ElementsEnabled[Object] = false
                    CleanupBatch += 1
                    if CleanupBatch <= MaxCleanupPerFrame then
...
function ESPLibrary:AddESP(Parameters)
    ...
    self.LastColorUpdate[Object] = Parameters.Color  -- FPS FIX: init color cache
...
function ESPLibrary:UpdateObjectColor(Object, Color)
    ...
    if self.Labels[Object] and self.Rainbow ~= true then self.Labels[Object].TextColor3 = Color end
    -- FPS FIX: invalidate color cache
    self.LastColorUpdate[Object] = nil
end
...
Interface.ApplyInfoTab = function(Window)
    ...
    local Changelog = {
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Frustum culling in ESP render</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Distance culling (800 stud max)</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Color change caching (no redundant property sets)</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Inline dead-object cleanup in render loop</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Dynamic queue batch size</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Throttled AutoInteract (10Hz, room-filtered)</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Split character handler into 3 frequency tiers</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Weak-key tables for PerformanceOriginals</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Stats overlay on Heartbeat (0.5s throttle)</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Aggressive cleaner (3s, dirty-flag)</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Prompt spatial index by room number</font>",
        "<font color='rgb(0, 255, 0)'>+ FPS FIX: Entity spawn debounce + no extra Humanoid</font>",
    }
...
-- ════════════════════════════════════════════════════════════════
-- FPS & PING STATS OVERLAY — FPS FIX: Heartbeat + 0.5s throttle
-- ════════════════════════════════════════════════════════════════
...
StatsFrame.Size = UDim2.new(0, 160, 0, 58)
...
-- FPS FIX: use Heartbeat (not RenderStepped), update labels every 0.5s
...
RunService.Heartbeat:Connect(function(Delta)
    ...
    if Now - LastPingCheck >= 1 then
        ...
        local PingColor = CurrentPing <= 100 and Color3.fromRGB(0, 170, 255) or CurrentPing <= 200 and Color3.fromRGB(255, 170, 0) or Color3.fromRGB(255, 60, 60)
...
local Options = Library.Options
-- ════════════════════════════════════════════════════════════════
-- PART 2: Variables, Tables, Notifications, Utilities, UI
-- ════════════════════════════════════════════════════════════════
...
local StairwellAntinoiseConnection = nil

-- ════════════════════════════════════════════════════════════════
-- PERFORMANCE LOCALS — FPS FIX: weak-key tables to prevent leaks
-- ════════════════════════════════════════════════════════════════
local PerformanceOriginals = {
    Particles = setmetatable({}, { __mode = "k" }),
    Trails = setmetatable({}, { __mode = "k" }),
    Lights = setmetatable({}, { __mode = "k" }),
    Textures = setmetatable({}, { __mode = "k" }),
    Materials = setmetatable({}, { __mode = "k" }),
    Reflectance = setmetatable({}, { __mode = "k" }),
    Effects = setmetatable({}, { __mode = "k" }),
    GlobalShadows = nil,
    Lighting = {},
}
local PerformanceCleanupThread = nil
local PerformanceCleanupActive = false
...
local FakePrompts = {}
local Functions = {}
local PartProperties = {}  -- FPS FIX: weak keys to auto-collect
setmetatable(PartProperties, { __mode = "k" })
...
-- FPS FIX: room-indexed prompt lookup for O(1) AutoInteract
local PromptsByRoom = {}
local PromptsNoRoom = {}
...
-- ════════════════════════════════════════════════════════════════
local Entities = {
    ["StemsEntity"] = { Alias = "Balls", NotifyMessage = { Title = "Balls", Body = "Balls." } },
    ["NoiseModel"] = { Alias = "Noise", NotifyMessage = { Title = "Entity 'Noise' has spawned.", Body = "Dont let it touch you." } },
    ["Creak"] = { Alias = "Creak", NotifyMessage = { Title = "Entity 'Creak' has spawned.", Body = "Dont touch him." } },
    ["DronesStampede"] = { Alias = "DronesStampede", NotifyMessage = { Title = "Entity 'Drones Stampede' has spawned.", Body = "Find a hiding spot." } },
    ["TellerRig"] = { Alias = "Teller", NotifyMessage = { Title = "Entity 'Teller' has spawned.", Body = "Dont worry, hes only annoying." } },
    ["Scribbles"] = { Alias = "Scribbles", NotifyMessage = { Title = "Entity 'Scribbles' has spawned.", Body = "Find a hiding spot." } },
    ["BashMoving"] = { Alias = "Bash", NotifyMessage = { Title = "Entity 'Bash' has spawned.", Body = "Find a hiding spot." } },
    ...
    ["Groundskeeper"] = { Alias = "Groundskeeper", NotifyMessage = { Title = "Entity 'Groundskeeper' has spawned.", Body = "Avoid stepping on the grass." } },
    ...
    ["GloombatSwarm"] = { Alias = "Gloombat Swarm", NotifyMessage = { Title = "Entity 'Gloombat Swarm' has spawned.", Body = "Keep all light sources turned off." } },
    ...
    ["MonumentEntity"] = { Alias = "Monument", NotifyMessage = { Title = "Entity 'Monument' has spawned.", Body = "It can't move while you are looking at it." } },
...
-- ════════════════════════════════════════════════════════════════
local ItemNames = {
    ["DinkyLamp"] = "Lamp", ["BottleCrate"] = "18+ Bottles", ["GweenSodaPack"] = "Gween Soda Pack",
    ["BrokenMonitor"] = "Broken Monitor", ["JerryCan"] = "Jerry Can",
    ["SallyToyObtain"] = "Sally Toy", ["Leftovers"] = "Lunch Box", ["HoneyPot"] = "Honey Pot",
    ["FihFlakes"] = "Fih Food", ["SecretCD"] = "CD Disc", ["Pizza"] = "Pizza",
    ["PaperPlanePickup"] = "Paper Plane", ["Lighter"] = "Lighter", ["Flashlight"] = "Flashlight",
    ["Lockpick"] = "Lockpicks", ["Vitamins"] = "Vitamins", ["Bandage"] = "Bandage",
    ["StarVial"] = "Starlight Vial", ["StarBottle"] = "Starlight Bottle", ["StarJug"] = "Starlight Barrel",
    ["Shakelight"] = "Gummy Flashlight", ["Straplight"] = "Straplight", ["Bulklight"] = "Spotlight",
    ["Battery"] = "Battery", ["Candle"] = "Candle", ["Crucifix"] = "Crucifix", ["CrucifixWall"] = "Crucifix",
    ["Glowsticks"] = "Glowstick", ["SkeletonKey"] = "Skeleton Key", ["Candy"] = "Candy",
    ["ShieldMini"] = "Mini Shield Potion", ["ShieldBig"] = "Big Shield Potion",
    ["BandagePack"] = "Bandage Pack", ["BatteryPack"] = "Battery Pack",
    ["RiftCandle"] = "Moonlight Candle", ["LaserPointer"] = "Laser Pointer",
    ["HolyGrenade"] = "Holy Hand Grenade", ["Shears"] = "Shears", ["Smoothie"] = "Smoothie",
    ["Cheese"] = "Cheese", ["Bread"] = "Bread", ["AlarmClock"] = "Alarm Clock",
    ["RiftSmoothie"] = "Moonlight Smoothie", ["GweenSoda"] = "Gween Soda",
    ["GlitchCube"] = "Glitch Fragment", ["Scanner"] = "Tablet", ["Bomb"] = "Bomb",
    ["Knockbomb"] = "Knockbomb", ["Nanner"] = "Nanner", ["BigBomb"] = "Big Bomb",
    ["SnakeBox"] = "Hiding Box", ["GoldGun"] = "Golden Gun", ["StopSign"] = "Stop Sign",
    ["TipJar"] = "Tip Jar", ["Lantern"] = "Lantern", ["IronKey"] = "Iron Key",
    ...
    ["AloeVera"] = "Aloe Vera", ["Donut"] = "Donut", ["Lotus"] = "Lotus",
...
function NotificationLibrary:Notify(TitleText, Desc, Delay)
    task.spawn(function()
        ...
        Services.TweenService:Create(Notification, TweenInfo.new(1, Enum.EasingStyle.Exponential), { Position = UDim2.new(1, -370, 0, Notification.Position.Y.Offset) }):Play()

        task.wait(0.25)
        if typeof(Delay) == "Instance" then
            Delay.Destroying:Wait()
        else
            Services.TweenService:Create(Line, TweenInfo.new(Delay - 0.25, Enum.EasingStyle.Linear), { Size = UDim2.new(0, 400, 0, 3) }):Play()
        ...
        for _, Object in Container:GetChildren() do
            if Object.Name == "Notification" and Object:GetAttribute("ID") and Object:GetAttribute("ID") > NotifId and Object:GetAttribute("Destroying") ~= true and Object.Position.Y.Offset ~= 60 then
                local NewY = Object:GetAttribute("CurrentPosition").Y.Offset - 60
                Object:SetAttribute("CurrentPosition", UDim2.new(1, -450, 0, NewY))
                Services.TweenService:Create(Object, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Position = UDim2.new(1, -370, 0, NewY) }):Play()
            end
        end
...
if not LiveModifiers then LiveModifiers = Instance.new("Folder") end
if not FloorReplicated then FloorReplicated = Instance.new("Folder") end
...
local EntityDistances = {
    ["RushMoving"]    = 85, ["Scribbles"] = 100, ["BashMoving"] = 150,
    ["DronesStampede"] = 100, ["AmbushMoving"] = 150, ["A60"] = 125,
...
Functions.GetNearestHidingSpot = function()
    ...
    local function GetHidePrompt(Object)
        ...
        for _, Child in Object:GetDescendants() do
            if (Child:IsA("ProximityPrompt") or Child:IsA("InteractPrompt")) and (Child.Name == "HidePrompt" or Child.Name == "HidingPrompt" or IsHidingSpotName(Child.Name)) then
                return Child
            end
        end
...
task.spawn(function()
    ...
    Icon.Image = "rbxassetid://85965233139643"
...
local Tabs = {
    ...
    Performance = Window:AddTab("Performance", "rbxassetid://113052394116996"),
}

-- ════════════════════════════════════════════════════════════════
-- GENERAL TAB — CHARACTER
-- ════════════════════════════════════════════════════════════════
Groupboxes.General_Character = Tabs.General:AddLeftGroupbox("Character")
Groupboxes.General_Character:AddSlider("SpeedBoostSlider", { Text = "Speed Boost", Min = 0, Max = 100, Default = 0, Rounding = 0, Compact = true })
Groupboxes.General_Character:AddToggle("SpeedBoostToggle", { Text = "Enable Speed Boost", Default = false, Tooltip = "Increases your walkspeed by the specified amount." })
Groupboxes.General_Character:AddToggle("FlyToggle", { Text = "Fly", Default = false, Tooltip = "Allows you to freely fly around the map." })
Toggles.FlyToggle:AddKeyPicker("FlyKeybind", { Text = "Fly", Default = "F", Mode = "Toggle", SyncToggleState = true })
Groupboxes.General_Character:AddSlider("FlySpeed", { Text = "Fly Speed", Min = 0, Max = 115, Default = 20, Rounding = 0, Compact = true })
Groupboxes.General_Character:AddDivider()
Groupboxes.General_Character:AddToggle("NoclipToggle", { Text = "Noclip", Default = false, Tooltip = "Allows your character to pass through solid objects." })
Groupboxes.General_Character:AddToggle("RemoveClosetDelay", { Text = "Remove Closet Delay", Default = false, Tooltip = "Removes the short window where you can't exit out of a closet after the animation finishes." })
Groupboxes.General_Character:AddToggle("RemoveAcceleration", { Text = "Remove Acceleration", Default = false, Tooltip = "Prevents your character from sliding while moving." })
...
Toggles.NoclipToggle:AddKeyPicker("NoclipKeybind", { Text = "Noclip", Default = "N", Mode = "Toggle", SyncToggleState = true })
...
Groupboxes.General_Character:AddDivider()
Groupboxes.General_Character:AddToggle("EnableCharacterJump", { Text = "Enable Jumping", Default = false, Tooltip = "Allows your character to jump." })
Groupboxes.General_Character:AddToggle("EnableCharacterSlide", { Text = "Enable Sliding", Default = false, Tooltip = "Allows your character to slide." })
Groupboxes.General_Character:AddToggle("InfiniteJumps", { Text = "Infinite Jumps", Default = false, Tooltip = "Allows you to jump while in the air." })
...
Groupboxes.General_Self:AddToggle("DoorReachToggle", { Text = "Door Reach", Default = false, Tooltip = "Allows you to open doors from further away." })
Groupboxes.General_Self:AddToggle("DisableIdleKick", { Text = "Disable Idle Kick", Default = false, Tooltip = "Prevents the kick from being idle for 20 minutes." })
...
Groupboxes.General_Self:AddSlider("PromptReachSlider", { Text = "Prompt Reach Multiplier", Min = 1, Max = 2, Default = 1, Rounding = 1, Compact = true })
Groupboxes.General_Self:AddToggle("InstantPrompts", { Text = "Instant Prompts", Default = false, Tooltip = "Allows you to trigger all prompts instantly." })
Groupboxes.General_Self:AddToggle("PromptClip", { Text = "Prompt Clip", Default = false, Tooltip = "Allows you to interact with prompts through walls." })
...
Groupboxes.Self_Automation:AddToggle("AutoBreakerBox", { Text = "Auto Breaker Box", Default = false, Tooltip = "Automatically solves the breaker box." })
Groupboxes.Self_Automation:AddToggle("AutoSolveAnchors", { Text = "Auto Solve Anchors", Default = false, Tooltip = "Automatically enters the correct code into anchors when you are near them." })
...
Groupboxes.Self_Automation:AddToggle("AutoUnlockPadlockToggle", { Text = "Auto Unlock Padlock", Default = false, Tooltip = "Automatically enters the code into the library padlock." })
Groupboxes.Self_Automation:AddSlider("AutoUnlockPadlockSlider", { Text = "Unlock Distance", Min = 1, Max = 50, Default = 10, Rounding = 0, Compact = true })
Groupboxes.Self_Automation:AddToggle("AutoLibraryGuessCode", { Text = "Guess Library Code", Default = false, Tooltip = "Attempts to guess the library code, but collecting some books is also necessary." })
Groupboxes.Self_Automation:AddDivider()
Groupboxes.Self_Automation:AddToggle("AutoInteractToggle", { Text = "Auto Interact", Default = false, Tooltip = "Automatically triggers nearby prompts." })
Toggles.AutoInteractToggle:AddKeyPicker("AutoInteractKeybind", { Text = "Auto Interact", Default = "R", Mode = Library.IsMobile and "Toggle" or "Hold", SyncToggleState = true })
...
Groupboxes.Self_Automation:AddDivider()
Groupboxes.Self_Automation:AddToggle("AutoClosetToggle", { Text = "Auto Closet", Default = false, Tooltip = "Automatically hides in a nearby closet when an entity is near." })
Toggles.AutoClosetToggle:AddKeyPicker("AutoClosetKeybind", { Text = "Auto Closet", Default = "Q", Mode = "Toggle", SyncToggleState = true })
...
Groupboxes.Self_Automation:AddToggle("SpectateEntityToggle", { Text = "Spectate Entity", Default = false, Tooltip = "Spectates the entity while auto hiding." })
Groupboxes.Self_Automation:AddDropdown("SpecateEntityMode", { Values = {"Player to Entity", "Entity to Player"}, Default = 1, AllowNull = true })

-- ════════════════════════════════════════════════════════════════
-- GENERAL TAB — MISC
-- ════════════════════════════════════════════════════════════════
Groupboxes.Self_Misc = Tabs.General:AddRightGroupbox("Miscellaneous")
Groupboxes.Self_Misc:AddButton({ Text = "Play Again", Tooltip = "Makes you join a new run, click again to cancel.", DoubleClick = true, Func = function() RemotesFolder.PlayAgain:FireServer() end })
Groupboxes.Self_Misc:AddButton({ Text = "Return to Lobby", Tooltip = "Makes you teleport back to the lobby.", DoubleClick = true, Func = function() RemotesFolder.Lobby:FireServer() end })
Groupboxes.Self_Misc:AddButton({ Text = "Revive", Tooltip = "Makes you revive, if you have a revive and haven't already revived in this run.", DoubleClick = true, Func = function() RemotesFolder.Revive:FireServer() end })
...
Groupboxes.Debug:AddButton({ Text = "Void", Tooltip = "Teleports your character to Y -120.", Func = function()
    if not Character then return end
    local Pivot = Character:GetPivot()
    for _ = 1, 22 do Character:PivotTo(Pivot + Vector3.new(0, -120 - Pivot.Position.Y, 0)) end
end })
Groupboxes.Debug:AddButton({ Text = "Exit Closet", Tooltip = "Exits the current closet.", Func = function()
    if RemotesFolder and RemotesFolder:FindFirstChild("CamLock") then RemotesFolder.CamLock:FireServer() end
end })

-- ════════════════════════════════════════════════════════════════
-- EXPLOITS TAB — LEFT: BYPASS / SOLVE
-- ════════════════════════════════════════════════════════════════
...
Groupboxes.Exploits_Bypass:AddToggle("BypassBanana",         { Text = "Bypass Banana",           Default = false, Tooltip = "Prevents 'Banana Peel' from slipping you up." })
Groupboxes.Exploits_Bypass:AddToggle("BypassJeff",           { Text = "Bypass Jeff",             Default = false, Tooltip = "Prevents 'Jeff the Killer' from stabbing you." })
...
-- ════════════════════════════════════════════════════════════════
-- EXPLOITS TAB — RIGHT: BYPASS GROUPBOX
-- ════════════════════════════════════════════════════════════════
...
Toggles.InfCrucifix:OnChanged(function(Value)
    ...
    InfCrucifixConnection = RunService.RenderStepped:Connect(function()
        if not Toggles.InfCrucifix.Value then
            if InfCrucifixConnection then InfCrucifixConnection:Disconnect() end
            return
        end
...
-- ════════════════════════════════════════════════════════════════
-- EXPLOITS TAB — RIGHT: REMOVE GROUPBOX
-- ════════════════════════════════════════════════════════════════
...
Toggles.RemoveHalt:OnChanged(function(Value)
    if Modules and Modules.Shade then
        Modules.Shade.Name = Value and "Shade_Disabled" or "Shade"
    end
end)
Toggles.RemoveA90:OnChanged(function(Value)
    if Modules and Modules.A90 then Modules.A90.Name = Value and "A90_Disabled" or "A90" end
end)
Toggles.RemoveDread:OnChanged(function(Value)
    if Modules and Modules.Dread then Modules.Dread.Name = Value and "Dread_Disabled" or "Dread" end
end)
...
Toggles.HonchoCorrectBoxESP:OnChanged(function(Value)
    ...
    for _, Object in pairs(HonchoESPObjects) do
        if Object and Object.Parent then Functions.RemoveESP(Object) end
    end
    ...
    local function ProcessHonchoRoom(Room)
        ...
        for _, Desc in ipairs(Room:GetDescendants()) do
            if Desc.Name == "ArchivesPackageDeposit" then
                ...
                if BoxID ~= nil then BoxIDs[BoxID] = true end
            end
        end
        if next(BoxIDs) == nil then HonchoProcessedRooms[Room] = nil return end
...
Toggles.ForgetMeNotSolver:OnChanged(function(Value)
    ...
    local function Run(Room)
        ...
        while ForgetMeNotRunning and NextRoom.Parent and Door:GetAttribute("Opened") ~= true do
            ...
            if Char then
                if Hidden:IsA("BasePart") then Char:PivotTo(Hidden.CFrame)
                elseif Hidden:IsA("Model") then Char:PivotTo(Hidden:GetPivot()) end
            end
        ...
        if ForgetMeNotRunning and Door:GetAttribute("Opened") == true then
            local Char = GetCharacter()
            if Char then
                for _ = 1, 4 do Char:PivotTo(CFrame.new(0, -120, 0)) end
...
Groupboxes.Stairwell_Misc:AddToggle("AntiNoise", {
    Text = "Anti Noise",
    Default = false,
...
Toggles.AntiNoise:OnChanged(function(value)
    if StairwellAntinoiseConnection then
        StairwellAntinoiseConnection:Disconnect()
        StairwellAntinoiseConnection = nil
    end
    if not value then return end

    StairwellAntinoiseConnection = RunService.PreSimulation:Connect(function(dt)
        ...
        if Globals.OriginalGetMoveVector then
            inputVector = Globals.OriginalGetMoveVector()
        else
...
-- ════════════════════════════════════════════════════════════════
-- PERFORMANCE TAB — EFFECTS CLEANER
-- ════════════════════════════════════════════════════════════════
Groupboxes.Performance_Cleaner = Tabs.Performance:AddLeftGroupbox("Effects Cleaner")
Groupboxes.Performance_Cleaner:AddToggle("PerfRemoveLingeringEffects", {
    Text = "Remove Lingering Effects",
    Default = false,
    Tooltip = "Continuously removes left-over effects after entities despawn."
})
Groupboxes.Performance_Cleaner:AddSlider("PerfCleanupInterval", {
    Text = "Cleanup Interval (s)",
    Min = 1, Max = 30, Default = 5, Rounding = 0, Compact = true
})
Groupboxes.Performance_Cleaner:AddDropdown("PerfCleanupCategories", {
    Text = "Clean Categories",
    Values = { "ParticleEmitters", "Beams", "Trails", "Sounds", "Attachments", "Explosions", "Fire", "Smoke", "Debris" },
    Default = { "ParticleEmitters", "Beams", "Trails", "Sounds", "Explosions", "Fire", "Smoke" },
    Multi = true, AllowNull = true
})
Groupboxes.Performance_Cleaner:AddDivider()
Groupboxes.Performance_Cleaner:AddToggle("PerfRemoveEntityDebris", {
    Text = "Remove Entity Debris",
    Default = false,
    Tooltip = "Deletes dead/jumpscare props left behind by entities."
})
Groupboxes.Performance_Cleaner:AddButton({
    Text = "Run Cleanup Now",
    Tooltip = "Manually triggers one cleanup sweep.",
    Func = function()
        if Functions and Functions.PerformCleanup then
            Functions.PerformCleanup()
            Functions.Notify({ Title = "Cleanup complete." })
        end
    end
})

local EffectCleanupBlacklistNames = {
    "Camera", "Humanoid", "HumanoidRootPart", "Head",
    "Collision", "CollisionPart", "CollisionClone", "CollisionPartClone",
    "Blur", "DepthOfField", "SunRays", "ColorCorrection",
    "RushHub", "PromptContainer",
}

local function IsEffectSafeToRemove(Object)
    if not Object or not Object.Parent then return false end
    if Object:IsDescendantOf(Character) then return false end
    if Object:IsDescendantOf(Camera) then return false end
    if Object:IsDescendantOf(Services.CoreGui) then return false end
    if Object:IsDescendantOf(LocalPlayer.PlayerGui) then return false end
    if Object:IsDescendantOf(Services.Lighting) then return false end
    for _, Name in ipairs(EffectCleanupBlacklistNames) do
        if Object.Name == Name then return false end
    end
    if Object:IsA("Sound") and Object.SoundId == "" then return false end
    if Object:IsA("Sound") and Object.Playing then return false end
    if Object:IsA("BasePart") and Object.Parent and Object.Parent.Name == "Drops" then return false end
    return true
end

Functions.PerformCleanup = function()
    local Categories = Options.PerfCleanupCategories.Value or {}
    local Count = 0
    for _, Object in ipairs(Services.Workspace:GetDescendants()) do
        if not IsEffectSafeToRemove(Object) then continue end
        local ShouldRemove = false
        if Categories["ParticleEmitters"] and Object:IsA("ParticleEmitter") then
            if not Object.Enabled then ShouldRemove = true end
        elseif Categories["Beams"] and Object:IsA("Beam") then
            if not Object.Enabled then ShouldRemove = true end
        elseif Categories["Trails"] and Object:IsA("Trail") then
            if not Object.Enabled then ShouldRemove = true end
        elseif Categories["Sounds"] and Object:IsA("Sound") then
            if not Object.Playing and Object.TimePosition == 0 and Object.Parent ~= Character then ShouldRemove = true end
        elseif Categories["Attachments"] and Object:IsA("Attachment") then
            if not Object.Parent or not Object.Parent:IsA("BasePart") then ShouldRemove = true end
        elseif Categories["Explosions"] and Object:IsA("Explosion") then
            ShouldRemove = true
        elseif Categories["Fire"] and Object:IsA("Fire") then
            if not Object.Enabled then ShouldRemove = true end
        elseif Categories["Smoke"] and Object:IsA("Smoke") then
            if not Object.Enabled then ShouldRemove = true end
        end
    ...
    if Toggles.PerfRemoveEntityDebris.Value then
        for _, Object in ipairs(Services.Workspace:GetChildren()) do
            if Object:IsA("Model") and Entities[Object.Name] then
                local IsActive = false
                if Object.PrimaryPart then
                    if LocalPlayer:DistanceFromCharacter(Object.PrimaryPart.Position) < 1000 then IsActive = true end
                end
...
Toggles.PerfRemoveLingeringEffects:OnChanged(function(Value)
    if PerformanceCleanupThread then
        task.cancel(PerformanceCleanupThread)
        PerformanceCleanupThread = nil
    end
    if not Value then
        PerformanceCleanupActive = false
        return
    end
    PerformanceCleanupActive = true
    PerformanceCleanupThread = task.spawn(function()
        while PerformanceCleanupActive and Toggles.PerfRemoveLingeringEffects.Value do
            local Interval = Options.PerfCleanupInterval.Value or 5
            task.wait(Interval)
            if not PerformanceCleanupActive then break end
            pcall(Functions.PerformCleanup)
        end
    end)
end)

-- ════════════════════════════════════════════════════════════════
-- PERFORMANCE TAB — GRAPHICS
-- ════════════════════════════════════════════════════════════════
Groupboxes.Performance_Graphics = Tabs.Performance:AddRightGroupbox("Graphics Downgrade")
Groupboxes.Performance_Graphics:AddToggle("PerfLowGraphics", { Text = "Lower Graphics", Default = false, Tooltip = "Reduces rendering quality." })
Groupboxes.Performance_Graphics:AddDropdown("PerfGraphicsLevel", { Text = "Quality Level", Values = { "Low", "Medium", "Very Low" }, Default = "Low" })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableParticles", { Text = "Disable All Particles", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableTrails", { Text = "Disable Trails & Beams", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableLights", { Text = "Disable Dynamic Lights", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableTextures", { Text = "Hide Decals & Textures", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableShadows", { Text = "Disable Global Shadows", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisablePostEffects", { Text = "Disable Post-Processing", Default = false })
Groupboxes.Performance_Graphics:AddToggle("PerfDisableFog", { Text = "Disable Fog", Default = false })

PerformanceOriginals.GlobalShadows = Services.Lighting.GlobalShadows

local function ApplyGraphicsDowngrade()
    local Level = Options.PerfGraphicsLevel.Value
    local Enabled = Toggles.PerfLowGraphics.Value
    task.spawn(function()
        local Descendants = Services.Workspace:GetDescendants()
        local Count = 0
        for _, Object in ipairs(Descendants) do
            if Object:IsA("BasePart") then
                if PerformanceOriginals.Materials[Object] == nil then
                    PerformanceOriginals.Materials[Object] = Object.Material
                    PerformanceOriginals.Reflectance[Object] = Object.Reflectance
                end
                if Enabled then
                    if Level == "Very Low" then
                        Object.Material = Enum.Material.SmoothPlastic
                        Object.Reflectance = 0
                    elseif Level == "Low" then
                        if Object.Material ~= Enum.Material.SmoothPlastic and Object.Material ~= Enum.Material.Plastic then
                            Object.Material = Enum.Material.Plastic
                        end
                        Object.Reflectance = 0
                    else
                        Object.Reflectance = math.min(Object.Reflectance, 0.1)
                    end
                else
                    if PerformanceOriginals.Materials[Object] then Object.Material = PerformanceOriginals.Materials[Object] end
                    if PerformanceOriginals.Reflectance[Object] then Object.Reflectance = PerformanceOriginals.Reflectance[Object] end
                end
            end
            Count += 1
...
local function ApplyParticleToggle(Value)
    task.spawn(function()
        local Count = 0
        for _, Object in ipairs(Services.Workspace:GetDescendants()) do
            if Object:IsA("ParticleEmitter") then
                if PerformanceOriginals.Particles[Object] == nil then PerformanceOriginals.Particles[Object] = Object.Enabled end
                Object.Enabled = Value and false or PerformanceOriginals.Particles[Object]
            end
            Count += 1
            if Count % 500 == 0 then task.wait() end
        end
...
local function ApplyTrailsToggle(Value)
    task.spawn(function()
        local Count = 0
        for _, Object in ipairs(Services.Workspace:GetDescendants()) do
            if Object:IsA("Trail") or Object:IsA("Beam") then
                if PerformanceOriginals.Trails[Object] == nil then PerformanceOriginals.Trails[Object] = Object.Enabled end
                Object.Enabled = Value and false or PerformanceOriginals.Trails[Object]
            end
            Count += 1
            if Count % 500 == 0 then task.wait() end
        end
...
local function ApplyLightsToggle(Value)
    task.spawn(function()
        local Count = 0
        for _, Object in ipairs(Services.Workspace:GetDescendants()) do
            if Object:IsA("PointLight") or Object:IsA("SpotLight") or Object:IsA("SurfaceLight") then
                if PerformanceOriginals.Lights[Object] == nil then PerformanceOriginals.Lights[Object] = Object.Enabled end
                Object.Enabled = Value and false or PerformanceOriginals.Lights[Object]
            end
            Count += 1
            if Count % 500 == 0 then task.wait() end
        end
...
local function ApplyTexturesToggle(Value)
    task.spawn(function()
        local Count = 0
        for _, Object in ipairs(Services.Workspace:GetDescendants()) do
            if Object:IsA("Decal") or Object:IsA("Texture") then
                if PerformanceOriginals.Textures[Object] == nil then PerformanceOriginals.Textures[Object] = Object.Transparency end
                Object.Transparency = Value and 1 or PerformanceOriginals.Textures[Object]
            end
            Count += 1
            if Count % 500 == 0 then task.wait() end
        end
...
local function ApplyPostEffectsToggle(Value)
    task.spawn(function()
        for _, Object in ipairs(Services.Lighting:GetDescendants()) do
            if Object:IsA("PostEffect") then
                if PerformanceOriginals.Effects[Object] == nil then PerformanceOriginals.Effects[Object] = Object.Enabled end
                Object.Enabled = Value and false or PerformanceOriginals.Effects[Object]
            end
        end
        if Services.Workspace.CurrentCamera then
            for _, Object in ipairs(Services.Workspace.CurrentCamera:GetChildren()) do
                if Object:IsA("PostEffect") then
                    if PerformanceOriginals.Effects[Object] == nil then PerformanceOriginals.Effects[Object] = Object.Enabled end
                    Object.Enabled = Value and false or PerformanceOriginals.Effects[Object]
                end
...
local function ApplyFogToggle(Value)
    if Value then
        PerformanceOriginals.Lighting.FogEnd = Services.Lighting.FogEnd
        PerformanceOriginals.Lighting.FogStart = Services.Lighting.FogStart
        Services.Lighting.FogEnd = 10000000
        Services.Lighting.FogStart = 10000000
    else
        if PerformanceOriginals.Lighting.FogEnd then Services.Lighting.FogEnd = PerformanceOriginals.Lighting.FogEnd end
        if PerformanceOriginals.Lighting.FogStart then Services.Lighting.FogStart = PerformanceOriginals.Lighting.FogStart end
    end
end

Toggles.PerfLowGraphics:OnChanged(function() ApplyGraphicsDowngrade() end)
Options.PerfGraphicsLevel:OnChanged(function() ApplyGraphicsDowngrade() end)
Toggles.PerfDisableParticles:OnChanged(ApplyParticleToggle)
Toggles.PerfDisableTrails:OnChanged(ApplyTrailsToggle)
Toggles.PerfDisableLights:OnChanged(ApplyLightsToggle)
Toggles.PerfDisableTextures:OnChanged(ApplyTexturesToggle)
Toggles.PerfDisablePostEffects:OnChanged(ApplyPostEffectsToggle)
Toggles.PerfDisableShadows:OnChanged(function(Value)
    Services.Lighting.GlobalShadows = Value and false or PerformanceOriginals.GlobalShadows
end)
Toggles.PerfDisableFog:OnChanged(ApplyFogToggle)

-- ════════════════════════════════════════════════════════════════
-- PERFORMANCE TAB — FPS
-- ════════════════════════════════════════════════════════════════
Groupboxes.Performance_FPS = Tabs.Performance:AddRightGroupbox("FPS")
Groupboxes.Performance_FPS:AddToggle("PerfFPSUnlock", { Text = "Unlock FPS", Default = false, Tooltip = "Removes the default FPS cap." })
Groupboxes.Performance_FPS:AddSlider("PerfFPSTarget", { Text = "Target FPS", Min = 30, Max = 360, Default = 240, Rounding = 0, Compact = true })
Groupboxes.Performance_FPS:AddToggle("PerfFPSBoost", { Text = "FPS Boost", Default = false, Tooltip = "Disables extraneous rendering systems." })
Groupboxes.Performance_FPS:AddDivider()
local PerfFPSLabel = Groupboxes.Performance_FPS:AddLabel("Live FPS: 60")

local function TrySetFPS(FPS)
    if not FPS or FPS <= 0 then FPS = 60 end
    local Success = false
    if setfpscap then
        pcall(function() setfpscap(FPS) Success = true end)
    end
    if not Success and typeof(setfflag) == "function" then
        pcall(function()
...
Toggles.PerfFPSUnlock:OnChanged(function(Value)
    if Value then TrySetFPS(Options.PerfFPSTarget.Value or 240)
    else TrySetFPS(60) end
end)

Options.PerfFPSTarget:OnChanged(function(Value)
    if Toggles.PerfFPSUnlock.Value then TrySetFPS(Value) end
end)

Toggles.PerfFPSBoost:OnChanged(function(Value)
    if Value then
        pcall(function()
...
-- FPS FIX: use Heartbeat for FPS counter, throttle to 1s
task.spawn(function()
    local FrameCount = 0
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if Now - LastUpdate >= 1 then
            ...
            FrameCount = 0
            LastUpdate = Now
            pcall(function() PerfFPSLabel:SetText("Live FPS: " .. FPS) end)
        end
    end)
end)
...
-- ════════════════════════════════════════════════════════════════
Groupboxes.Performance_Misc = Tabs.Performance:AddRightGroupbox("Misc")
Groupboxes.Performance_Misc:AddToggle("PerfDisableAmbientMusic", { Text = "Disable Ambient Music", Default = false })
Groupboxes.Performance_Misc:AddToggle("PerfDisableChatSounds", { Text = "Disable Chat Sounds", Default = false })
Groupboxes.Performance_Misc:AddDivider()
Groupboxes.Performance_Misc:AddButton({
    Text = "Force Full Cleanup",
    Tooltip = "Destroys every stale effect across the workspace.",
    Func = function()
        if Functions and Functions.PerformCleanup then
            local Count = Functions.PerformCleanup()
            Functions.Notify({ Title = "Cleaned " .. (Count or 0) .. " effects." })
        end
    end
})
Groupboxes.Performance_Misc:AddButton({
    Text = "Reset Graphics",
    Tooltip = "Restores all original materials, textures, and effects.",
    Func = function()
        Toggles.PerfLowGraphics:SetValue(false)
        Toggles.PerfDisableParticles:SetValue(false)
        Toggles.PerfDisableTrails:SetValue(false)
        Toggles.PerfDisableLights:SetValue(false)
        Toggles.PerfDisableTextures:SetValue(false)
        Toggles.PerfDisablePostEffects:SetValue(false)
        Toggles.PerfDisableShadows:SetValue(false)
        Toggles.PerfDisableFog:SetValue(false)
        Functions.Notify({ Title = "Graphics restored." })
    end
})
-- ════════════════════════════════════════════════════════════════
-- PART 3: Visuals Tab, Floors Tab, Hooks, Character Handler
-- ════════════════════════════════════════════════════════════════
...
Toggles.AmbientToggle:OnChanged(function(Value)
    local Room = CurrentRooms:FindFirstChild(tostring(LocalPlayer:GetAttribute("CurrentRoom")))
...
Toggles.DisableGlitchJumpscare:OnChanged(function(Value)
    if Modules and Modules.Glitch then Modules.Glitch.Name = Value and "Glitch_Disabled" or "Glitch" end
end)
Toggles.DisableTimothyJumpscare:OnChanged(function(Value)
    if Modules and Modules.SpiderJumpscare then Modules.SpiderJumpscare.Name = Value and "SpiderJumpscare_Disabled" or "SpiderJumpscare" end
end)
Toggles.DisableVoidJumpscare:OnChanged(function(Value)
    if Modules and Modules.Void then Modules.Void.Name = Value and "Void_Disabled" or "Void" end
end)
...
Groupboxes.Visuals_ESP_Settings:AddSlider("ESPRenderLimit",             { Text = "Render Limit",              Min = 30, Max = 240, Default = 120, Rounding = 0, Compact = true })
...
RushHub.ESPLibrary:SetFillTransparency(0.75)
...
RushHub.ESPLibrary:SetRenderLimit(120)
RushHub.ESPLibrary:SetFadeTime(0.25)
RushHub.ESPLibrary:SetTextSize(20)
RushHub.ESPLibrary:SetFont(Enum.Font.Highway)
...
RushHub.ESPLibrary:SetArrowRadius(250)
RushHub.ESPLibrary:SetDistanceSizeRatio(0.8)
...
-- ════════════════════════════════════════════════════════════════
-- CHARACTER HANDLER — FPS FIX: 3-TIER SPLIT
-- Tier 1: RenderStepped — critical visual (camera, collision, noclip)
-- Tier 2: Heartbeat 10Hz — moderate (crouch, eyes, ambient, speed)
-- Tier 3: Heartbeat 2Hz  — rare (animations, proximity checks)
-- ════════════════════════════════════════════════════════════════
local CharacterOldConnectionKeys = {
    ...
    "FootstepHandler", "ModerateHandler", "RareHandler"
}
...
Functions.HandleCharacter = function(NewCharacter)
    ...
    -- ═══════════════════════════════════════════════════════════
    -- TIER 1: RenderStepped — critical visual updates (every frame)
    -- Camera, collision, noclip, fly, velocity, third person, spectate
    -- ═══════════════════════════════════════════════════════════
    ...
    -- ═══════════════════════════════════════════════════════════
    -- TIER 2: Heartbeat 10Hz — moderate updates (every 0.1s)
    -- Crouch fire, eyes check, ambient, camera shake, viewmodel, self killed
    -- ═══════════════════════════════════════════════════════════
    local ModerateAccumulator = 0
    Connections.ModerateHandler = Services.RunService.Heartbeat:Connect(function(Delta)
        ModerateAccumulator += Delta
        if ModerateAccumulator < 0.1 then return end
        ModerateAccumulator = 0
        ...
        local MF = Globals.MainUI:FindFirstChild("MainFrame")
        if MF then
            local Effects = MF.Healthbar:FindFirstChild("Effects")
    ...
    -- ═══════════════════════════════════════════════════════════
    -- TIER 3: Heartbeat 2Hz — rare updates (every 0.5s)
    -- Animation check, proximity player, lagging check
    -- ═══════════════════════════════════════════════════════════
    local RareAccumulator = 0
    Connections.RareHandler = Services.RunService.Heartbeat:Connect(function(Delta)
        RareAccumulator += Delta
        if RareAccumulator < 0.5 then return end
        RareAccumulator = 0
...
-- ════════════════════════════════════════════════════════════════
-- PART 4: HandleObject, EntitySpawn, Prompts, AutoInteract, Unload
-- ════════════════════════════════════════════════════════════════
...
-- FPS FIX: helper to add prompts to room index
local function IndexPrompt(Object)
    local RoomNum = tonumber(Object:GetAttribute("ParentRoom"))
    if RoomNum then
        if not PromptsByRoom[RoomNum] then PromptsByRoom[RoomNum] = {} end
        table.insert(PromptsByRoom[RoomNum], Object)
    else
        table.insert(PromptsNoRoom, Object)
    end
end
...
Functions.HandleObject = function(Object)
    ...
    elseif Name == "ArchivesPackageDeposit" then
        if Toggles.ObjectiveESPToggle.Value then Functions.AddESP({ Object = Object, Text = "BoxDeposit", Color = Options.ObjectiveESPColor.Value }, true) end
        table.insert(Objects.Objectives, Object)
    ...
    elseif Object.ClassName == "ProximityPrompt" and not Object:GetAttribute("FakePrompt") then
        ...
        if Functions.CheckCompatability({"fireproximityprompt"}) and Floor ~= "OldHotel" and Floor ~= "Fools" then
            ...
            if IsLockPrompt then
                ...
                -- FPS FIX: index the fake prompt
                IndexPrompt(FakePrompt)
...
local AllowedInstances = {
    ...
    ArchivesPackageDeposit=true, SalvageChute=true, StairwellFireAlarm=true, ShoppingCart=true,
}
...
-- ════════════════════════════════════════════════════════════════
-- PRIMARY WORKSPACE SCAN — FPS FIX (cooperative, yields every 250)
-- ════════════════════════════════════════════════════════════════
task.spawn(function()
    local Descendants = Services.Workspace:GetDescendants()
    local Processed = 0
    for _, Object in ipairs(Descendants) do
        pcall(Functions.QueueObject, Object)
        Processed += 1
...
-- ════════════════════════════════════════════════════════════════
-- INSTANCE HANDLER — FPS FIX (debounced DescendantAdded)
-- ════════════════════════════════════════════════════════════════
local PendingQueue = {}
local QueueScheduled = false
Connections.InstanceHandler = Services.Workspace.DescendantAdded:Connect(function(Object)
    local Name = Object.Name
    if not AllowedInstances[Name] and Object.ClassName ~= "ProximityPrompt" then
        if not ItemNames[Name] then
            local Lower = string.lower(Name)
            if not string.find(Lower, "hidingspot", 1, true) then
                return
            end
        end
    end

    table.insert(PendingQueue, Object)
    if QueueScheduled then return end
    QueueScheduled = true
    task.defer(function()
        QueueScheduled = false
        for _, Obj in PendingQueue do
            Functions.QueueObject(Obj)
        end
...
-- ════════════════════════════════════════════════════════════════
-- QUEUE PROCESSOR — FPS FIX (dynamic batch size)
-- ════════════════════════════════════════════════════════════════
Globals.QueueDone = true
local QueueProcessing = false

Connections.QueueConnection = Services.RunService.Heartbeat:Connect(function()
    if QueueProcessing then return end
    local QueueLen = #Globals.ObjectQueue
    if QueueLen == 0 then return end
    
    -- FPS FIX: dynamic batch size based on queue length
    local BatchSize = QueueLen > 500 and 25 or QueueLen > 100 and 10 or QueueLen > 20 and 5 or 3
    
    QueueProcessing = true
    task.spawn(function()
        local Processed = 0
        while #Globals.ObjectQueue > 0 and Processed < BatchSize do
...
Functions.TriggerPrompt = function(Prompt)
    ...
    if Prompt.Parent.Parent and Prompt.Parent.Parent.Name == "MeldChord" then return end
...
-- ════════════════════════════════════════════════════════════════
-- AUTO INTERACT LOOP — FPS FIX: O(1) room-indexed lookup, 10Hz
-- ════════════════════════════════════════════════════════════════
Globals.LastAutoInteractFire = tick()
Globals.AutoInteractBusy = false
...
Connections.AutoInteract = Services.RunService.Heartbeat:Connect(function()
    ...
    if Globals.AutoInteractBusy then return end
    local Now = tick()
    if Now - Globals.LastAutoInteractFire < 0.1 then return end
    Globals.LastAutoInteractFire = Now

    local CurrentRoom = tonumber(LocalPlayer:GetAttribute("CurrentRoom"))
    local FoundPrompt = nil

    -- FPS FIX: check only prompts in current room first
    local RoomPrompts = PromptsByRoom[CurrentRoom]
    if RoomPrompts then
        for _, Prompt in RoomPrompts do
            if Prompt and Prompt.Parent and (Prompt.Parent:IsA("BasePart") or Prompt.Parent:IsA("Model")) then
                local Distance
    ...
    -- FPS FIX: if nothing in current room, check no-room prompts
    if not FoundPrompt then
        for _, Prompt in PromptsNoRoom do
            if Prompt and Prompt.Parent and (Prompt.Parent:IsA("BasePart") or Prompt.Parent:IsA("Model")) then
                local Distance
                if Prompt.Parent:IsA("BasePart") then
                    Distance = LocalPlayer:DistanceFromCharacter(Prompt.Parent.Position)
    ...
    if FoundPrompt then
        Globals.AutoInteractBusy = true
        task.spawn(function()
            pcall(Functions.TriggerPrompt, FoundPrompt)
            Globals.AutoInteractBusy = false
        end)
    end
end)
...
-- ════════════════════════════════════════════════════════════════
-- CLEANER — FPS FIX (task.spawn, 2s interval, cleans prompt indexes)
-- ════════════════════════════════════════════════════════════════
Connections.CleanerActive = true
task.spawn(function()
    while Connections.CleanerActive do
        task.wait(2)
        if not Connections.CleanerActive then break end
        for ArrayName, Array in Objects do
            if type(Array) ~= "table" then continue end
            local I = #Array
            while I >= 1 do
                local Object = Array[I]
                if Object == nil or (typeof(Object) == "Instance" and not Object:IsDescendantOf(Services.Workspace)) then
        ...
        -- FPS FIX: clean prompt indexes
        for RoomNum, PromptList in pairs(PromptsByRoom) do
            local I = #PromptList
            while I >= 1 do
            ...
            if #PromptList == 0 then
                PromptsByRoom[RoomNum] = nil
            end
        end
        
        local I = #PromptsNoRoom
        while I >= 1 do
            local Prompt = PromptsNoRoom[I]
            if not Prompt or not Prompt.Parent then
                table.remove(PromptsNoRoom, I)
            end
            I -= 1
        end
    end
...
-- ════════════════════════════════════════════════════════════════
-- PROMPT FIXER — FPS FIX (task.spawn, 2s interval, spread over frames)
-- ════════════════════════════════════════════════════════════════
Connections.PromptFixerActive = true
task.spawn(function()
    while Connections.PromptFixerActive do
        task.wait(2)
        if not Connections.PromptFixerActive then break end
        local Count = 0
        for _, Prompt in Objects.Prompts do
            if Prompt and Prompt.Parent and Prompt:HasTag("DisableWhenEnabledOnClient") then
...
LocalPlayer.CharacterAdded:Connect(function(NewCharacter)
    ...
    if Connections.ModerateHandler then
        Connections.ModerateHandler:Disconnect()
        Connections.ModerateHandler = nil
    end
    if Connections.RareHandler then
        Connections.RareHandler:Disconnect()
        Connections.RareHandler = nil
    end
    task.wait(0.5)
...
Library:OnUnload(function()
    ...
    -- Stop performance cleanup
    PerformanceCleanupActive = false
    if PerformanceCleanupThread then task.cancel(PerformanceCleanupThread) PerformanceCleanupThread = nil end

    -- Stop cleaners
    Connections.CleanerActive = false
    Connections.PromptFixerActive = false

    -- Restore graphics
    pcall(function()
        if Toggles.PerfLowGraphics.Value then Toggles.PerfLowGraphics:SetValue(false) end
        if Toggles.PerfDisableParticles.Value then Toggles.PerfDisableParticles:SetValue(false) end
        if Toggles.PerfDisableTrails.Value then Toggles.PerfDisableTrails:SetValue(false) end
        if Toggles.PerfDisableLights.Value then Toggles.PerfDisableLights:SetValue(false) end
        if Toggles.PerfDisableTextures.Value then Toggles.PerfDisableTextures:SetValue(false) end
        if Toggles.PerfDisablePostEffects.Value then Toggles.PerfDisablePostEffects:SetValue(false) end
        if Toggles.PerfDisableShadows.Value then Toggles.PerfDisableShadows:SetValue(false) end
        if Toggles.PerfDisableFog.Value then Toggles.PerfDisableFog:SetValue(false) end
    end)

    -- Stop FPS unlock
    if Toggles.PerfFPSUnlock.Value then
        pcall(function() Toggles.PerfFPSUnlock:SetValue(false) end)
    end
    ...
    if Globals.ManipulateBody then Globals.ManipulateBody:Destroy() end
    ...
    if TimeShowerLabel then TimeShowerLabel:Destroy() end
