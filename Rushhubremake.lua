...
Groupboxes.Exploits_Bypass:AddToggle("EntityGhostToggle", { Text = "Entity Bypass (Rush/Ambush/A-60/A-120/Glitch)", Default = false, Tooltip = "Lets all lethal rush-type entities pass through you without dealing damage." })
...
Groupboxes.Exploits_BypassRight:AddToggle("InfCrucifix", {
    ...
    Tooltip = "Drops and picks up the Crucifix near entities to prevent it from breaking.",
})
...
Toggles.InfCrucifix:OnChanged(function(Value)
    ...
    InfCrucifixConn = RunService.Heartbeat:Connect(function()
        ...
        local CollisionPart = Player.Character and Player.Character:FindFirstChild("CollisionPart")
        if not CollisionPart then return end

        local Tool = Player.Character:FindFirstChildOfClass("Tool")
        if not Tool or Tool.Name ~= "Crucifix" then return end

        InfCrucifixRaycastParams.FilterDescendantsInstances = {Player.Character}
        ...
        for _, Entity in ipairs(Services.Workspace:GetChildren()) do
            ...
            if RayResult and RayResult.Instance:IsDescendantOf(Entity) then
                local Dist = (CollisionPart.Position - Entity.PrimaryPart.Position).Magnitude
        ...
        if NearestEntity and NearestDist < InfCrucifixDropTable[NearestEntity.Name] then
            task.spawn(function()
                local Success = pcall(function()
                    Services.ReplicatedStorage.RemotesFolder.DropItem:FireServer(Tool)
                end)
                ...
                elseif Prompt and RushHub.Environment.fireproximityprompt then
