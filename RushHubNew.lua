-- RushHub for Doors
-- past 1/5
-- Refactored: Entity bypass fix, Stairwell flesh bypass, trash teleport, inf crucifix fix, tab icons, auto-farm knobs, flesh door ESP
...
local function RunTest(Name, Test, InternalName)
    ...
    if BrokenList and table.find(BrokenList, Name) then
        AddResult(Name, "X " .. Name .. " skipped", false)
        Failed = Failed + 1
        return
    end
    ...
    if not TargetGlobal then
        AddResult(Name, "X " .. Name .. " nil", false)
        Failed = Failed + 1
        return
    end
    ...
    task.spawn(function()
        ...
        if not TimedOut then
            if Success then
                ...
                AddResult(Name, "X " .. Name .. " failed: " .. tostring(Result), false)
                Failed = Failed + 1
            end
        end
        Finished = true
    ...
    while not Finished do
        Time = Time + 1
        if Time > 100 then
            AddResult(Name, "X " .. Name .. " timeout", false)
            Failed = Failed + 1
            TimedOut = true
            break
        end
...
RunTest("cloneref", function()
    ...
    local Clone = Global.cloneref(Test)
    assert(typeof(Clone) == "Instance", "Should return an Instance")
    assert(Test ~= Clone, "Clone should not be equal to original")
    Test.Name = "Test"
...
-- ════════════════════════════════════════════════════════════════
-- ESP LIBRARY (inlined) — POLISHED + FPS FIX + FLESH DOOR ESP
-- ════════════════════════════════════════════════════════════════
local ESPLibrary = {
...
local function DestroyObjectData(Object)
    ...
    for Idx = #ESPLibrary.FleshDoorObjects, 1, -1 do
        if ESPLibrary.FleshDoorObjects[Idx] == Object then table.remove(ESPLibrary.FleshDoorObjects, Idx) break end
    end
end
...
function ESPLibrary:StartRenderLoop()
    ...
    self.RenderConnection = ESPRunService.Heartbeat:Connect(function(Delta)
        ...
        if self.FleshDoorESPEnabled then
            for _, Obj in ipairs(self.FleshDoorObjects) do
                if self.ElementsEnabled[Obj] == true then
...
function ESPLibrary:AddESP(Parameters)
    ...
    if Parameters.FleshDoor then
        self.FleshDoorESPEnabled = true
        table.insert(self.FleshDoorObjects, Object)
    end
...
function ESPLibrary:AddFleshDoorESP(Door)
    if not Door or self.ElementsEnabled[Door] == true then return end
    self:AddESP({
        Object = Door,
        Text = "Flesh Door",
...
function ESPLibrary:RemoveFleshDoorESP(Door)
    if not Door then return end
    self:RemoveESP(Door)
    for Idx = #self.FleshDoorObjects, 1, -1 do
        if self.FleshDoorObjects[Idx] == Door then
            table.remove(self.FleshDoorObjects, Idx)
            break
        end
    end
end
...
Interface.ApplyInfoTab = function(Window)
    ...
    local InfoTab = Window:AddTab("Info", "rbxassetid://89916252002728")
    ...
    local Credits = {
        "<font color='rgb(138, 43, 226)'>ar0sla — Script Creator</font>",
        "<font color='rgb(50, 205, 50)'>" .. LocalPlayer.Name .. " — User</font>",
    }
    ...
    local Changelog = {
        "<font color='rgb(0, 255, 0)'>+ Fixed Rush/Ambush entity bypass (comprehensive)</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Stairwell Flesh Bypass</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Trash Teleport to player in Stairwell</font>",
        "<font color='rgb(0, 255, 0)'>+ Fixed Infinite Crucifix bug</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Auto-Farm Knobs via Queue Teleport</font>",
        "<font color='rgb(0, 255, 0)'>+ Added Flesh Door ESP</font>",
        "<font color='rgb(0, 255, 0)'>+ Updated tab icons</font>",
        "<font color='rgb(0, 255, 0)'>+ Stairwell tab lime icon</font>",
        "<font color='rgb(0, 255, 0)'>+ Removed big labels</font>",
        "<font color='rgb(0, 255, 0)'>+ All functions now work on Stairwell entities</font>",
    }
...
Interface.ApplySettingsTab = function(Window)
    ...
    MenuGroup:AddToggle("QueueOnTeleport", {
        ...
        Tooltip = "Automatically reloads RushHub when you teleport.",
...
-- ════════════════════════════════════════════════════════════════
-- FPS & PING STATS OVERLAY — v3 REDESIGN
-- ════════════════════════════════════════════════════════════════
...
-- Stairwell locals
local StairwellTrashConn = nil
local StairwellFleshBypassConn = nil
local StairwellFleshDoorScanConn = nil
local StairwellFleshDoors = {}
local KnobFarmQueueTeleportConn = nil
...
-- Comprehensive entity list including Stairwell entities
local Entities = {
    ["StemsEntity"] = {
        Alias = "Balls",
    ...
    ["TellerRig"] = {
        Alias = "Teller",
        NotifyMessage = { Title = "Entity 'Teller' has spawned.", Body = "Don't worry." }
    },
    ["Scribbles"] = {
        Alias = "Scribbles",
    ...
    ["FleshFigure"] = {
        Alias = "Flesh Figure",
        NotifyMessage = { Title = "Entity 'Flesh Figure' has spawned.", Body = "Flesh entity detected in Stairwell." }
    },
    ["FleshStalk"] = {
        Alias = "Flesh Stalk",
        NotifyMessage = { Title = "Entity 'Flesh Stalk' has spawned.", Body = "Flesh entity detected in Stairwell." }
    },
    ["Dread"] = {
        Alias = "Dread",
        NotifyMessage = { Title = "Entity 'Dread' has spawned.", Body = "Find a hiding spot." }
    },
    ["Halt"] = {
        Alias = "Halt",
        NotifyMessage = { Title = "Entity 'Halt' has spawned.", Body = "Walk forward when it says." }
    },
    ["Screech"] = {
        Alias = "Screech",
        NotifyMessage = { Title = "Entity 'Screech' has spawned.", Body = "Look at it." }
    },
}
...
Globals.DoorsNotify = function(NotifyOptions)
    ...
    Achievement.Frame.Details.Title.Text = NotifyOptions.Title or "Sem Titulo"
    Achievement.Frame.Details.Desc.Text = NotifyOptions.Description or "Sem Descricao"
...
-- Enhanced entity distances — now includes Stairwell entities
...
Functions.GetNearestEntity = function(CheckDisabled, List, UseRaycasting)
    ...
    -- Also check workspace children for entities not in Objects.Entities yet
    for _, Entity in ipairs(Services.Workspace:GetChildren()) do
        if Entity and Entity.Parent and Entity:IsA("Model") and EntityDistances[Entity.Name] and Entity.PrimaryPart then
            local EntityData = Entities[Entity.Name]
            if EntityData then
                if List and List[EntityData.Alias] then continue end
                local Distance = LocalPlayer:DistanceFromCharacter(Entity.PrimaryPart.Position)
...
Functions.GetNearestFigure = function()
    local Nearest = { Distance = math.huge, Object = nil }
    local FigureNames = { FigureRig = true, FigureRagdoll = true, Figure = true, FleshFigure = true }
...
-- ════════════════════════════════════════════════════════════════
-- FLESH DOOR SCANNER — Scans for flesh doors in Stairwell
-- ════════════════════════════════════════════════════════════════
Functions.ScanFleshDoors = function()
    for _, Room in ipairs(CurrentRooms:GetChildren()) do
        for _, Desc in ipairs(Room:GetDescendants()) do
            if Desc:IsA("BasePart") or Desc:IsA("Model") then
                local Name = string.lower(Desc.Name)
                if Name:find("flesh") and (Name:find("door") or Name:find("passage") or Name:find("wall")) then
                    if not table.find(Objects.FleshDoors, Desc) then
                        table.insert(Objects.FleshDoors, Desc)
                        if Toggles.FleshDoorESP and Toggles.FleshDoorESP.Value then
                            RushHub.ESPLibrary:AddFleshDoorESP(Desc)
                        end
                        Desc.Destroying:Once(function()
                            RushHub.ESPLibrary:RemoveFleshDoorESP(Desc)
                            local Pos = table.find(Objects.FleshDoors, Desc)
                            if Pos then table.remove(Objects.FleshDoors, Pos) end
                        end)
                    end
                end
            end
...
Functions.StartFleshDoorScan = function()
    if StairwellFleshDoorScanConn then return end
    Functions.ScanFleshDoors()
    StairwellFleshDoorScanConn = CurrentRooms.DescendantAdded:Connect(function(Desc)
        local Name = string.lower(Desc.Name)
        if (Desc:IsA("BasePart") or Desc:IsA("Model")) and Name:find("flesh") and (Name:find("door") or Name:find("passage") or Name:find("wall")) then
            if not table.find(Objects.FleshDoors, Desc) then
                table.insert(Objects.FleshDoors, Desc)
                if Toggles.FleshDoorESP and Toggles.FleshDoorESP.Value then
                    RushHub.ESPLibrary:AddFleshDoorESP(Desc)
...
Functions.StopFleshDoorScan = function()
    if StairwellFleshDoorScanConn then
        StairwellFleshDoorScanConn:Disconnect()
        StairwellFleshDoorScanConn = nil
    end
    for _, Door in ipairs(Objects.FleshDoors) do
        RushHub.ESPLibrary:RemoveFleshDoorESP(Door)
    end
    table.clear(Objects.FleshDoors)
end

-- ════════════════════════════════════════════════════════════════
-- STAIRWELL TRASH TELEPORT — Brings trash to player in Stairwell subfloor
-- ════════════════════════════════════════════════════════════════
Functions.StartStairwellTrashTeleport = function()
    if StairwellTrashConn then
        StairwellTrashConn:Disconnect()
    end

    StairwellTrashConn = Services.RunService.Heartbeat:Connect(function()
        if not Toggles.StairwellTrashTeleport or not Toggles.StairwellTrashTeleport.Value then return end
        if not LocalPlayer.Character then return end
        local HRP = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if not HRP then return end

        local Stairwell = workspace:FindFirstChild("Stairwell")
        if not Stairwell then return end

        local TrashFolder = Stairwell:FindFirstChild("Trash")
        if not TrashFolder then return end

        for _, Trash in ipairs(TrashFolder:GetChildren()) do
            if Trash:IsA("BasePart") then
                Trash.CFrame = HRP.CFrame
            elseif Trash:IsA("Model") then
                Trash:PivotTo(HRP.CFrame)
            end
        end
    end)
end

Functions.StopStairwellTrashTeleport = function()
    if StairwellTrashConn then
        StairwellTrashConn:Disconnect()
        StairwellTrashConn = nil
    end
end

-- ════════════════════════════════════════════════════════════════
-- STAIRWELL FLESH BYPASS — Bypasses flesh entities in Stairwell
-- ════════════════════════════════════════════════════════════════
Functions.StartStairwellFleshBypass = function()
    if StairwellFleshBypassConn then
        StairwellFleshBypassConn:Disconnect()
    end

    StairwellFleshBypassConn = Services.RunService.Heartbeat:Connect(function()
        if not Toggles.StairwellFleshBypass or not Toggles.StairwellFleshBypass.Value then return end

        local Stairwell = workspace:FindFirstChild("Stairwell")
        if not Stairwell then return end

        for _, Desc in ipairs(Stairwell:GetDescendants()) do
            if Desc:IsA("BasePart") then
                local Name = string.lower(Desc.Name)
                if Name:find("flesh") or Name:find("figure") or Name:find("stalk") then
                    Desc.CanTouch = false
        ...
        for _, Entity in ipairs(Services.Workspace:GetChildren()) do
            if Entity:IsA("Model") then
                ...
                if EName == "FleshFigure" or EName == "FleshStalk" or EName:find("Flesh") then
...
Functions.StopStairwellFleshBypass = function()
    if StairwellFleshBypassConn then
        StairwellFleshBypassConn:Disconnect()
        StairwellFleshBypassConn = nil
    end
end

-- ════════════════════════════════════════════════════════════════
-- WINDOW & TABS CREATION — UPDATED ICONS
-- ════════════════════════════════════════════════════════════════
local Window = Library:CreateWindow({
    ...
    MenuFadeTime = 0,
    CornerRadius = 6,
    BackgroundImage = "rbxassetid://94041871724354",
})
...
-- Updated tab icons per Maker's specifications
local Tabs = {
    General   = Window:AddTab("General", "house"),
    Exploits  = Window:AddTab("Exploits", "rbxassetid://72384911282519"),
    Visuals   = Window:AddTab("Visuals", "rbxassetid://91849915276447"),
    Floors    = Window:AddTab("Floors", "rbxassetid://83697952186962"),
    Archives  = Window:AddTab("Archives", "rbxassetid://123768259663270"),
    Stairwell = Window:AddTab("Stairwell", "rbxassetid://80017304328364"),
}

-- Apply lime color to Stairwell tab icon
task.spawn(function()
    task.wait(0.3)
    local TabBar = Library.ScreenGui and Library.ScreenGui:FindFirstChild("Window")
        and Library.ScreenGui.Window:FindFirstChild("TabWindow")
    if TabBar then
        for _, Tab in ipairs(TabBar:GetChildren()) do
            if Tab:IsA("Frame") or Tab:IsA("TextButton") then
                local Icon = Tab:FindFirstChild("Icon", true) or Tab:FindFirstChildOfClass("ImageLabel")
                if Icon and Icon.Image == "rbxassetid://80017304328364" then
...
-- ════════════════════════════════════════════════════════════════
-- GENERAL TAB — CHARACTER GROUPBOX (labels removed)
-- ════════════════════════════════════════════════════════════════
...
Groupboxes.General_Character:AddToggle("RemoveAcceleration", {
    Text = "Remove Acceleration", Default = false
})
...
Groupboxes.General_Self:AddToggle("DoorReachToggle", {
    Text = "Door Reach", Default = false
})
Groupboxes.General_Self:AddToggle("DisableIdleKick", {
    Text = "Disable Idle Kick", Default = false
})
...
Groupboxes.General_Self:AddToggle("InstantPrompts", {
    Text = "Instant Prompts", Default = false
})
    ...
    if Value and CurrentRooms:FindFirstChild("ElevatorBreaker", true) then
        if not Globals.BreakerBoxInteracted then
            if not Globals.BreakerBoxNotified then
                Functions.Notify({ Title = "Interact with the breaker box." })
...
Groupboxes.Self_Automation:AddToggle("AutoHeartbeatMinigame", {
    Text = "Auto Heartbeat Minigame", Default = false,
...
Groupboxes.Self_Automation:AddToggle("AutoUnlockPadlockToggle", {
    Text = "Auto Unlock Padlock", Default = false
})
...
Groupboxes.Self_Automation:AddToggle("AutoInteractToggle", {
    Text = "Auto Interact", Default = false
})
...
Groupboxes.Self_Automation:AddToggle("SpectateEntityToggle", {
    Text = "Spectate Entity", Default = false
})
...
-- ════════════════════════════════════════════════════════════════
-- ════════════════════════════════════════════════════════════════
-- PART 2: Exploits, Anti-Lag, Archives+Honcho, Stairwell Start
-- ════════════════════════════════════════════════════════════════
...
Groupboxes.Exploits_Bypass:AddToggle("EntityGhostToggle", {
    Text = "Entity Bypass",
    Default = false,
    Tooltip = "All rush-type entities pass through you without damage."
})

-- Comprehensive rush entity names — includes Stairwell entities
local RushEntityNames = {
    RushMoving=true, AmbushMoving=true, A60=true, A120=true,
    GlitchRush=true, GlitchAmbush=true, BackdoorRush=true,
    BashMoving=true, Scribbles=true, DronesStampede=true,
    CustomEntity=true, FrozenAmbush=true, Creak=true,
    NoiseModel=true, StemsEntity=true, SallyMoving=true,
    FleshFigure=true, FleshStalk=true, Dread=true,
}

-- SINGLE unified entity ghost handler — no duplicates
Connections.EntityGhostHandler = Services.RunService.Heartbeat:Connect(function()
    ...
    -- Check workspace children directly for entities not yet in table
    for _, Entity in ipairs(Services.Workspace:GetChildren()) do
        if Entity and Entity.Parent and RushEntityNames[Entity.Name] then
            for _, Part in ipairs(Entity:GetDescendants()) do
                if Part:IsA("BasePart") and Part.CanTouch then
                    Part.CanTouch = false
                end
    ...
    -- Check Stairwell folder for flesh entities
    local Stairwell = Services.Workspace:FindFirstChild("Stairwell")
    if Stairwell then
        for _, Desc in ipairs(Stairwell:GetDescendants()) do
...
Groupboxes.Exploits_Bypass:AddToggle("BypassGiggle",         { Text = "Bypass Giggle",           Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassDupe",           { Text = "Bypass Dupe",             Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassEyes",           { Text = "Bypass Eyes",             Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassLookman",        { Text = "Bypass Lookman",          Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassGloombatEggs",   { Text = "Bypass Gloombat Eggs",    Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassSeekObstructions", { Text = "Bypass Seek Obstructions", Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassVacuum",         { Text = "Bypass Vacuum",           Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassKillbricks",     { Text = "Bypass Killbricks",       Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassSeekingWall",    { Text = "Bypass Seeking Wall",     Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassSnare",          { Text = "Bypass Snare",            Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassBanana",         { Text = "Bypass Banana",           Default = false })
Groupboxes.Exploits_Bypass:AddToggle("BypassJeff",           { Text = "Bypass Jeff",             Default = false })
    ...
    for _, Object in Objects.Entities do
        if Object.Name == "DoorFake" or Object.Name == "FakeDoor" then
            ...
            if Object:FindFirstChild("Lock") then
                local UnlockPrompt = Object.Lock:FindFirstChild("UnlockPrompt")
                if UnlockPrompt then UnlockPrompt.Enabled = not Value end
            end
        end
    end
end)
...
Toggles.BypassVacuum:OnChanged(function(Value)
    for _, Object in Objects.Entities do
        if Object.Name == "SideroomSpace" then
            ...
            if Coll then Coll.CanCollide = Value Coll.CanTouch = not Value end
        end
    end
end)
...
Groupboxes.Archives_Misc:AddToggle("AntiRansom", { Text = "Anti Ransom", Default = false })
Groupboxes.Archives_Misc:AddToggle("AntiClosetTrash", { Text = "Anti Closet Trash", Default = false })
Groupboxes.Archives_Misc:AddToggle("ForgetMeNotSolver", { Text = "Forget Me Not Skipper", Default = false })
Groupboxes.Archives_Misc:AddToggle("TimeShower", { Text = "Time Shower", Default = false })
Groupboxes.Archives_Misc:AddToggle("BypassDronesStampede", { Text = "Stop Time/Anti Stampede", Default = false })
Groupboxes.Archives_Misc:AddToggle("HonchoCorrectBoxESP", { Text = "Honcho Correct Box ESP", Default = false })
...
Groupboxes.Archives_Bypasses:AddToggle("BypassWater", { Text = "Bypass Electric Water", Default = false })
Groupboxes.Archives_Bypasses:AddToggle("BypassAlma", { Text = "Bypass Alma", Default = false })
Groupboxes.Archives_Bypasses:AddToggle("BypassDrones", { Text = "Bypass Drones", Default = false })
Groupboxes.Archives_Bypasses:AddToggle("AntiScribbles", { Text = "Bypass Scribbles", Default = false })
    ...
    local function Run(Room)
        ...
        if ForgetMeNotRunning and Door:GetAttribute("Opened") == true then
            ...
            if Character then
                ...
                Functions.Notify({Title = "Spam Void In Debug If Stuck"})
            end
...
-- ════════════════════════════════════════════════════════════════
-- FIXED INFINITE CRUCIFIX
-- ════════════════════════════════════════════════════════════════
Groupboxes.Exploits_BypassRight:AddToggle("InfCrucifix", {
    Text = "Infinite Crucifix", Default = false,
    Tooltip = "Drops and picks up the Crucifix near entities to prevent breaking."
})
...
local InfCrucifixProcessing = false
...
Toggles.InfCrucifix:OnChanged(function(Value)
    ...
    InfCrucifixConn = RunService.Heartbeat:Connect(function()
        if not Toggles.InfCrucifix.Value then return end
        if InfCrucifixProcessing then return end

        local Player = game.Players.LocalPlayer
        if not Player then return end
        local Char = Player.Character
        if not Char then return end
        local CollPart = Char:FindFirstChild("CollisionPart")
        if not CollPart then return end

        local Tool = Char:FindFirstChildOfClass("Tool")
        if not Tool or Tool.Name ~= "Crucifix" then return end

        InfCrucifixRaycastParams.FilterDescendantsInstances = {Char}
        ...
        for _, Entity in ipairs(Services.Workspace:GetChildren()) do
            ...
            local Origin = CollPart.Position
            local Direction = Entity.PrimaryPart.Position - Origin
            local RayResult = Services.Workspace:Raycast(Origin, Direction, InfCrucifixRaycastParams)
            if RayResult and RayResult.Instance:IsDescendantOf(Entity) then
                local Dist = (CollPart.Position - Entity.PrimaryPart.Position).Magnitude
        ...
        -- Also check Objects.Entities table
        for _, Entity in ipairs(Objects.Entities) do
            if Entity and Entity.Parent then
                local MaxDist = InfCrucifixDropTable[Entity.Name]
                if not MaxDist or not Entity.PrimaryPart then continue end
        ...
        if NearestEntity and NearestDist < InfCrucifixDropTable[NearestEntity.Name] then
            InfCrucifixProcessing = true
            task.spawn(function()
                local Success = pcall(function()
                    local DropRemote = Services.ReplicatedStorage.RemotesFolder:FindFirstChild("DropItem")
                    if DropRemote then
                        DropRemote:FireServer(Tool)
                    end
                end)
                if not Success then
                    InfCrucifixProcessing = false
                    return
                end

                task.wait(0.54)

                local DropsFolder = Services.Workspace:FindFirstChild("Drops")
                if not DropsFolder then
                    InfCrucifixProcessing = false
                    return
                end
                local Dropped = DropsFolder:FindFirstChild("Crucifix")
                if not Dropped then
                    InfCrucifixProcessing = false
                    return
                end
                ...
                local Prompt = Dropped:FindFirstChildOfClass("ProximityPrompt")
                    or Dropped:FindFirstChild("ModulePrompt")
                if Prompt then
                    if Functions.FirePrompt then
                        Functions.FirePrompt(Prompt)
                    elseif RushHub.Environment.fireproximityprompt then
                        RushHub.Environment.fireproximityprompt(Prompt)
                    end
                end
...
Groupboxes.Exploits_BypassRight:AddToggle("CrouchSpoof", {
    Text = "Crouch Spoof", Default = false
})
...
Groupboxes.Exploits_Remove:AddToggle("RemoveScreech",  { Text = "Remove Screech",  Default = false })
Groupboxes.Exploits_Remove:AddToggle("RemoveHalt",     { Text = "Remove Halt",     Default = false })
Groupboxes.Exploits_Remove:AddToggle("RemoveA90",      { Text = "Remove A-90",     Default = false })
Groupboxes.Exploits_Remove:AddToggle("RemoveDread",    { Text = "Remove Dread",    Default = false })
Groupboxes.Exploits_Remove:AddToggle("RemoveSurge",    { Text = "Remove Surge",    Default = false })
Groupboxes.Exploits_Remove:AddDivider()
Groupboxes.Exploits_Remove:AddToggle("NoScreechDamage", { Text = "No Screech Damage", Default = false })
Groupboxes.Exploits_Remove:AddToggle("NoHaltDamage",    { Text = "No Halt Damage",    Default = false })
Groupboxes.Exploits_Remove:AddToggle("NoA90Damage",     { Text = "No A-90 Damage",    Default = false })
Groupboxes.Exploits_Remove:AddToggle("NoSurgeDamage",   { Text = "No Surge Damage",   Default = false })
...
Groupboxes.Exploits_Audio:AddToggle("RemoveFootstepSounds",    { Text = "Remove Footstep Sounds",    Default = false })
Groupboxes.Exploits_Audio:AddToggle("RemoveJamminMusic",       { Text = "Remove Jammin Music",       Default = false })
Groupboxes.Exploits_Audio:AddToggle("RemoveInteractingSounds", { Text = "Remove Interacting Sounds", Default = false })
...
-- Flesh Bypass toggle
Groupboxes.Stairwell_Misc:AddToggle("StairwellFleshBypass", {
    Text = "Flesh Bypass",
    Default = false,
    Tooltip = "Bypasses all flesh entities and obstacles in Stairwell."
})
Toggles.StairwellFleshBypass:OnChanged(function(Value)
    if Value then
        Functions.StartStairwellFleshBypass()
    else
        Functions.StopStairwellFleshBypass()
    end
end)

-- Trash Teleport toggle
Groupboxes.Stairwell_Misc:AddToggle("StairwellTrashTeleport", {
    Text = "Trash Teleport",
    Default = false,
    Tooltip = "Teleports all Stairwell trash to your character."
})
Toggles.StairwellTrashTeleport:OnChanged(function(Value)
    if Value then
        Functions.StartStairwellTrashTeleport()
    else
        Functions.StopStairwellTrashTeleport()
    end
end)

-- Flesh Door ESP toggle
Groupboxes.Stairwell_Misc:AddToggle("FleshDoorESP", {
    Text = "Flesh Door ESP",
    Default = false,
    Tooltip = "Highlights flesh doors in Stairwell."
})
Toggles.FleshDoorESP:OnChanged(function(Value)
    if Value then
        Functions.StartFleshDoorScan()
    else
        Functions.StopFleshDoorScan()
    end
end)
                ...
                if workspaceDropsFolder then
                    for _, itemToBring in ipairs(workspaceDropsFolder:GetChildren()) do
                        if itemToBring:IsA("Model") then
                            itemToBring:PivotTo(playerHRP.CFrame)
                        elseif itemToBring:IsA("BasePart") then
                            itemToBring.CFrame = playerHRP.CFrame
                        end
                    end
                end
            end
...
Groupboxes.Visuals_Camera:AddToggle("AmbientToggle", { Text = "Ambient", Default = false })
Groupboxes.Visuals_Camera:AddSlider("FieldOfView", { Text = "Field of View", Min = 1, Max = 120, Default = 70, Rounding = 0 })
Groupboxes.Visuals_Camera:AddToggle("FOVToggle", { Text = "Custom FOV", Default = false })
...
Groupboxes.Visuals_Camera:AddToggle("RemoveCameraShake", {
    Text = "Remove Camera Shake", Default = false,
...
Groupboxes.Visuals_Camera:AddToggle("RemoveCameraBobbing", {
    Text = "Remove Camera Bobbing", Default = false,
    Disabled = not Functions.CheckCompatability({"require"}), DisabledTooltip = Globals.IncompatibleMessage
})
Groupboxes.Visuals_Camera:AddToggle("RemoveCutscenes", { Text = "Remove Cutscenes", Default = false })
Groupboxes.Visuals_Camera:AddToggle("RemoveCameraFog", { Text = "Remove Fog", Default = false })
Groupboxes.Visuals_Camera:AddDivider()
Groupboxes.Visuals_Camera:AddToggle("ThirdPersonToggle", { Text = "Third Person", Default = false })
...
Groupboxes.Visuals_Camera:AddToggle("ThirdPersonWallCheck", { Text = "Wall Check", Default = false })
Groupboxes.Visuals_Camera:AddDivider()
...
Groupboxes.Visuals_Effects:AddToggle("TransparentHidingSpotsToggle", { Text = "Transparent Hiding Spots", Default = false })
...
Groupboxes.Visuals_Effects:AddToggle("DisableGlitchJumpscare",   { Text = "Disable Glitch Jumpscare",   Default = false })
Groupboxes.Visuals_Effects:AddToggle("DisableTimothyJumpscare",  { Text = "Disable Timothy Jumpscare",  Default = false })
Groupboxes.Visuals_Effects:AddToggle("DisableVoidJumpscare",     { Text = "Disable Void Jumpscare",     Default = false })
Groupboxes.Visuals_Effects:AddDivider()
Groupboxes.Visuals_Effects:AddToggle("DisableHideVignette",      { Text = "Disable Hide Vignette",      Default = false })
Groupboxes.Visuals_Effects:AddToggle("DisableFiredampEffect",    { Text = "Disable Firedamp Effect",    Default = false })
Groupboxes.Visuals_Effects:AddToggle("DisableEntityJumpscares",  { Text = "Disable Entity Jumpscares",  Default = false })
...
-- ════════════════════════════════════════════════════════════════
-- PART 3: Visuals ESP/Entities, Floors Tab, Main Handlers, Init
-- ════════════════════════════════════════════════════════════════
...
Groupboxes.Visuals_Entities:AddDropdown("EntityList", {
    Text = "Entity List",
    Values = { "Rush","Bash","Scribbles","Teller","DronesStampede","Creak","Noise","Balls","Ambush","Eyes","Halt","Blitz","Lookman","Gloombat Swarm","A-60","A-120","Sally","Jeff the Killer","Groundskeeper","Monument","AR0xMBUSH","RNIUSHCG==","Dread","Flesh Figure","Flesh Stalk" },
    Multi = true, AllowNull = true
})
...
Groupboxes.Visuals_Entities:AddToggle("NotifyItemsToggle",          { Text = "Notify Items",    Default = false })
Groupboxes.Visuals_Entities:AddToggle("NotifyItemsShowDistance",    { Text = "Show Distance",    Default = false })
Groupboxes.Visuals_Entities:AddDivider()
Groupboxes.Visuals_Entities:AddToggle("NotifyLibraryCode", { Text = "Notify Library Code", Default = false })
Groupboxes.Visuals_Entities:AddToggle("NotifyOxygen",      { Text = "Notify Oxygen Level", Default = false })
Groupboxes.Visuals_Entities:AddToggle("NotifyHasteTime",   { Text = "Notify Haste Time",   Default = false })
...
Groupboxes.Visuals_EntitySettings:AddToggle("EntityChatToggle", { Text = "Notify Chat", Default = false })
...
Groupboxes.Visuals_EntitySettings:AddToggle("NotifyPlaySound", { Text = "Play Sound", Default = true })
Groupboxes.Visuals_EntitySettings:AddToggle("NotifyKeepNotifications", { Text = "Keep Notifications", Default = false })
...
Groupboxes.Visuals_ESP_Toggles:AddToggle("ObjectiveESPToggle", { Text = "Objectives", Default = false })
...
Groupboxes.Visuals_ESP_Toggles:AddToggle("DoorESPToggle",       { Text = "Doors",        Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("HidingSpotESPToggle", { Text = "Hiding Spots", Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("PlayerESPToggle",     { Text = "Players",      Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("ChestESPToggle",      { Text = "Chests",       Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("ItemESPToggle",       { Text = "Items",        Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("CurrencyESPToggle",   { Text = "Currency",     Default = false })
Groupboxes.Visuals_ESP_Toggles:AddToggle("LadderESPToggle",     { Text = "Ladders",      Default = false })
Groupboxes.Visuals_ESP_Toggles:AddDivider()

Groupboxes.Visuals_ESP_Toggles:AddDropdown("EntityESPOptions", {
    Text = "Entity List",
    Values = { "Rush","Bash","Scribbles","Teller","DronesStampede","Creak","Noise","Balls","Ambush","Eyes","Dupe","Figure","Blitz","Lookman","Snare","Giggle","Gloombat Eggs","Grumble","A-60","A-120","Sally","Jeff the Killer","Groundskeeper","Mandrake Hole","Monument","Bramble","AR0xMBUSH","RNIUSHCG==","Flesh Figure","Flesh Stalk","Dread","Halt","Screech" },
    Multi = true, AllowNull = true
})
Groupboxes.Visuals_ESP_Toggles:AddToggle("EntityESPToggle",     { Text = "Entities",      Default = false })
...
local EntityESPLabels = {
    ...
    FleshFigure = "Flesh Figure", FleshStalk = "Flesh Stalk",
}
local NodeEntities = {
    ...
    Creak = true, Noise = true, Balls = true, Dread = true, ["Flesh Figure"] = true, ["Flesh Stalk"] = true,
    Halt = true, Screech = true
}
...
-- ESP Settings
Groupboxes.Visuals_ESP_Settings:AddToggle("ESPRainbow",     { Text = "Rainbow Effect", Default = false })
Groupboxes.Visuals_ESP_Settings:AddToggle("ESPShowDistance",{ Text = "Show Distance",  Default = true })
...
Groupboxes.Visuals_ESP_Settings:AddToggle("ESPTracersToggle",    { Text = "Enable Tracers", Default = false })
Groupboxes.Visuals_ESP_Settings:AddDivider()
...
Groupboxes.Visuals_ESP_Settings:AddToggle("ESPArrowsToggle",     { Text = "Enable Arrows", Default = false })
...
Groupboxes.Floors_Automation:AddToggle("AutoSteerMinecart", {
    Text = "Auto Steer Minecart", Default = false,
    Disabled = not Functions.CheckCompatability({"require"}), DisabledTooltip = Globals.IncompatibleMessage
})
...
Groupboxes.Floors_Automation:AddToggle("RoomsAutoWalk",             { Text = "Auto Rooms",       Default = false })
...
Groupboxes.Floors_Automation:AddToggle("RoomsAutoWalkIgnoreA60",    { Text = "Ignore A-60",      Default = false })
Groupboxes.Floors_Automation:AddToggle("RoomsAutoWalkShowPathToggle", { Text = "Show Path",       Default = false })
Groupboxes.Floors_Automation:AddToggle("RoomsAutoWalkSpoofFootsteps", {
    Text = "Spoof Footsteps", Default = false,
...
Groupboxes.Floors_Visuals:AddToggle("ShowSeekPathToggle", {
    Text = "Show Seek Path", Default = false,
    Risky = Floor ~= "Mines"
})
...
Groupboxes.Floors_Visuals:AddToggle("ShowEyestalkPathToggle", {
    Text = "Show Eyestalk Path", Default = false,
    Risky = Floor ~= "Garden"
})
...
Groupboxes.Floors_Bypass:AddToggle("RemoveSeekTrigger", {
    Text = "Delete Seek Trigger", Default = false,
...
Groupboxes.Floors_Bypass:AddToggle("RemoveFigure", {
    Text = "Delete Figure", Default = false,
...
Groupboxes.Floors_Bypass:AddToggle("AutoRevive", {
    Text = "Infinite Revives", Default = false,
    Risky = not (Floor == "Fools" or Floor == "OldHotel")
})
Groupboxes.Floors_Bypass:AddToggle("FigureGodmode", {
    Text = "Figure Godmode", Default = false,
    Risky = not (Floor == "Fools" or Floor == "OldHotel")
})
Groupboxes.Floors_Bypass:AddDivider()
Groupboxes.Floors_Bypass:AddToggle("RemoveBasementGate",  { Text = "Remove Basement Gate",   Default = false, Risky = not (Floor == "Fools" or Floor == "OldHotel") })
Groupboxes.Floors_Bypass:AddToggle("RemovePaintingsDoor", { Text = "Remove Paintings Door",  Default = false, Risky = not (Floor == "Fools" or Floor == "OldHotel") })
Groupboxes.Floors_Bypass:AddToggle("RemoveSkeletonDoor",  { Text = "Remove Skeleton Door",   Default = false, Risky = Floor ~= "Fools" })
...
-- ════════════════════════════════════════════════════════════════
-- FLOORS TAB — FARMING WITH AUTO-FARM KNOBS VIA QUEUE TELEPORT
-- ════════════════════════════════════════════════════════════════
Groupboxes.Floors_Farming = Tabs.Floors:AddLeftGroupbox("Farming")
Groupboxes.Floors_Farming:AddToggle("KnobFarm", {
    Text = "Knob Farm", Default = false,
    Tooltip = "Automatically gains knobs for you via queue teleport.",
})

Groupboxes.Floors_Farming:AddToggle("AutoFarmKnobsQueue", {
    Text = "Auto-Farm Knobs (Queue TP)",
    Default = false,
    Tooltip = "Farms knobs by teleporting to new servers automatically using queue_on_teleport."
})
...
-- Auto-Farm Knobs via Queue Teleport
Toggles.AutoFarmKnobsQueue:OnChanged(function(Value)
    if Value then
        Globals.AutoFarmKnobsActive = true
        Functions.Notify({Title = "Auto-Farm Knobs Started", Body = "Will teleport to new servers after each run."})
        
        -- Enable queue_on_teleport if available
        if queue_on_teleport then
            queue_on_teleport([[loadstring(game:HttpGet("https://raw.githubusercontent.com/deividcomsono/Obsidian/refs/heads/main/Library.lua"))()]])
        end
        
        -- Start the farm loop
        KnobFarmQueueTeleportConn = task.spawn(function()
            while Globals.AutoFarmKnobsActive do
                if not Toggles.AutoFarmKnobsQueue.Value then break end
                
                -- Wait until we're in a run
                if LocalPlayer:GetAttribute("Alive") == true and LatestRoom.Value >= 0 then
                    -- Collect gold
                    for _, GoldObj in ipairs(Objects.Currency) do
                        if GoldObj.Name == "GoldPile" and GoldObj.PrimaryPart then
                            local Dist = LocalPlayer:DistanceFromCharacter(GoldObj.PrimaryPart.Position)
                            if Dist < 500 then
                                Character:PivotTo(GoldObj:GetPivot())
                                task.wait(0.2)
                                local Prompt = GoldObj:FindFirstChildOfClass("ProximityPrompt") or GoldObj:FindFirstChild("ModulePrompt")
                                if Prompt then
                    ...
                    -- Wait for death or run completion
                    if LocalPlayer:GetAttribute("Alive") == false then
                        ...
                        -- If still dead, teleport to new server
                        if LocalPlayer:GetAttribute("Alive") ~= true then
                            Functions.Notify({Title = "Teleporting to new server...", Body = "Auto-farming knobs."})
                            task.wait(1)
        ...
        Globals.AutoFarmKnobsActive = false
        if KnobFarmQueueTeleportConn then
            pcall(function() task.cancel(KnobFarmQueueTeleportConn) end)
            KnobFarmQueueTeleportConn = nil
        end
        Functions.Notify({Title = "Auto-Farm Knobs Stopped"})
    end
end)
...
Functions.HandleCharacter = function(NewCharacter)
    ...
    Connections.AnticheatDisabler = Character:GetAttributeChangedSignal("Climbing"):Connect(function()
        if Character:GetAttribute("Climbing") == true and Toggles.DisableAnticheat.Value and not Globals.AnticheatDisabled then
            ...
            Functions.Notify({ Title = "Successfully disabled the anticheat." })
    ...
    Connections.AnticheatEnableDetector1 = RemotesFolder:WaitForChild("Cutscene").OnClientEvent:Connect(function(CutsceneName)
        if Globals.AnticheatDisabled and not CutsceneName:find("SewerSeek") then
            Globals.AnticheatDisabled = false
            Functions.Notify({ Title = "The anticheat has been re-enabled." })
        end
    end)
...
-- ════════════════════════════════════════════════════════════════
-- END OF PART 3 — Continue to Part 4 (Final)
-- ════════════════════════════════════════════════════════════════
-- ════════════════════════════════════════════════════════════════
-- PART 4: HandleObject, EntitySpawn, Prompts, AutoInteract, Unload, Init
-- ════════════════════════════════════════════════════════════════
...
Functions.HandleObject = function(Object)
    ...
    elseif Name == "Groundskeeper" then
        ...
        table.insert(Objects.Entities, Object)
    elseif Name == "Figure" or Name == "FigureRig" or Name == "FigureRagdoll" or Name == "FleshFigure" or Name == "FleshStalk" then
        for _, Part in Object:GetDescendants() do if Part:IsA("BasePart") then Part.CanTouch = false end end
        ...
        Connections.BreakerConnection = Object:WaitForChild("SurfaceGui").Frame.Code:GetPropertyChangedSignal("Text"):Connect(function()
            if Toggles.AutoBreakerBox.Value then
                if not Globals.BreakerBoxStartNotified and (Floor == "Fools" or Floor == "OldHotel") then
                    Functions.Notify({ Title = "Attempting to solve the breaker box." })
    ...
    elseif Name == "ElevatorCar" then
        local ElevConn = Object.DescendantAdded:Connect(function(Desc)
            if Toggles.AutoBreakerBox.Value and Desc.Name == "TouchInterest" and not Globals.BreakerBoxFinishedNotified then
                Functions.Notify({ Title = "Successfully solved the breaker box." })
...
local AllowedInstances = {
    ...
    FleshFigure=true, FleshStalk=true,
}
...
Library:OnUnload(function()
    ...
    local OldAmbient = CurrentRooms:FindFirstChild(tostring(LocalPlayer:GetAttribute("CurrentRoom"))) and CurrentRooms:FindFirstChild(tostring(LocalPlayer:GetAttribute("CurrentRoom"))):GetAttribute("Ambient") or Color3.new(1,1,1)
    ...
    if Globals.OriginalGetMoveVector then
        local ControlsMod = require(LocalPlayer.PlayerScripts.PlayerModule):GetControls()
        ControlsMod.GetMoveVector = Globals.OriginalGetMoveVector
    end
